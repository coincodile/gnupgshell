...
wxGnuPGShellSelectRecipient::wxGnuPGShellSelectRecipient(wxWindow *parent,
...
bool wxGnuPGShellSelectRecipient::Create(wxWindow *parent, wxWindowID id,
...
////@begin wxGnuPGShellSelectRecipient content construction
	...
	m_useKey = new wxRadioButton(itemDialog1, ID_RECEPIENT_ASYNC, _("Use key"),
			wxDefaultPosition, wxDefaultSize, wxRB_GROUP);
	m_useKey->SetValue(true);
	itemBoxSizer4->Add(m_useKey, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);

	wxStaticLine *itemStaticLine6 = new wxStaticLine(itemDialog1, wxID_STATIC,
			wxDefaultPosition, wxDefaultSize, wxLI_HORIZONTAL);
	itemBoxSizer4->Add(itemStaticLine6, 1, wxALIGN_CENTER_VERTICAL | wxALL, 5);

	m_recKeyList = new wxListView(itemDialog1, ID_RECIPIENT_KEYLIST,
			wxDefaultPosition, wxSize(400, 200), wxLC_REPORT | wxLC_SINGLE_SEL);
	itemBoxSizer2->Add(m_recKeyList, 0, wxGROW | wxALL, 5);

	wxBoxSizer *itemBoxSizer8 = new wxBoxSizer(wxHORIZONTAL);
	itemBoxSizer2->Add(itemBoxSizer8, 0, wxGROW | wxALL, 5);

	m_usePass = new wxRadioButton(itemDialog1, ID_RECEPIENT_SYNC,
			_("Use password"), wxDefaultPosition, wxDefaultSize, 0);
	...
	itemFlexGridSizer11->Add(itemStaticText12, 0,
			wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL | wxALL, 5);

	m_pass = new wxTextCtrl(itemDialog1, ID_RECEPIENT_PASS, _T(""),
			wxDefaultPosition, wxDefaultSize, wxTE_PASSWORD);
	m_pass->Enable(false);
	itemFlexGridSizer11->Add(m_pass, 0,
			wxGROW | wxALIGN_CENTER_VERTICAL | wxALL, 5);

	wxStaticText *itemStaticText14 = new wxStaticText(itemDialog1, wxID_STATIC,
			_("Repeat:"), wxDefaultPosition, wxDefaultSize, 0);
	itemFlexGridSizer11->Add(itemStaticText14, 0,
			wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL | wxALL, 5);

	m_repeat = new wxTextCtrl(itemDialog1, ID_RECEPIENT_REPEAT, _T(""),
			wxDefaultPosition, wxDefaultSize, wxTE_PASSWORD);
	m_repeat->Enable(false);
	itemFlexGridSizer11->Add(m_repeat, 0,
			wxGROW | wxALIGN_CENTER_VERTICAL | wxALL, 5);

	m_needArmor = new wxCheckBox(itemDialog1, ID_RECIPIENT_ARMOR, _("Armor"),
			wxDefaultPosition, wxDefaultSize, 0);
	m_needArmor->SetValue(true);
	itemBoxSizer2->Add(m_needArmor, 0, wxALIGN_LEFT | wxALL, 5);

	wxStaticLine *itemStaticLine17 = new wxStaticLine(itemDialog1,
	ID_STATICLINE5, wxDefaultPosition, wxDefaultSize, wxLI_HORIZONTAL);
	...
	wxButton *itemButton19 = new wxButton(itemDialog1, wxID_OK, _("&OK"),
			wxDefaultPosition, wxDefaultSize, 0);
	itemStdDialogButtonSizer18->AddButton(itemButton19);

	wxButton *itemButton20 = new wxButton(itemDialog1, wxID_CANCEL,
...
void wxGnuPGShellSelectRecipient::OnOkUpdate(wxUpdateUIEvent &event) {
	bool enable =
			m_useKey->GetValue() ?
					m_recKeyList->GetSelectedItemCount() > 0 :
					(m_pass->GetValue() != wxEmptyString
							&& m_pass->GetValue() == m_repeat->GetValue());
	event.Enable(enable);
}
...
void wxGnuPGShellSelectRecipient::FillKeyList() {
	...
	for (int i = 0; i < count; i++) {
		...
		if (key) {
			...
			if (i % 2) {
				// odd lines color
				m_recKeyList->SetItemBackgroundColour(i, cl);
			}
		}
	}
...
void wxGnuPGShellSelectRecipient::OnRECEPIENTASYNCSelected(
...
void wxGnuPGShellSelectRecipient::OnRECEPIENTSYNCSelected(
		wxCommandEvent &event) {
	m_pass->Enable();
	m_repeat->Enable();
