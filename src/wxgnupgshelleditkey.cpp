...
wxGnuPGShellEditKey::wxGnuPGShellEditKey(wxWindow *parent, wxWindowID id,
...
bool wxGnuPGShellEditKey::Create(wxWindow *parent, wxWindowID id,
		const wxString &caption, const wxPoint &pos, const wxSize &size,
		long style) {
...
////@begin wxGnuPGShellEditKey content construction
	...
	wxStaticText *itemStaticText4 = new wxStaticText(itemDialog1, wxID_STATIC,
			_("User name:"), wxDefaultPosition, wxSize(60, -1), wxALIGN_RIGHT);
	...
	wxStaticText *itemStaticText7 = new wxStaticText(itemDialog1, wxID_STATIC,
			_("Key ID:"), wxDefaultPosition, wxSize(60, -1), wxALIGN_RIGHT);
	...
	wxButton *itemButton9 = new wxButton(itemDialog1, ID_EDITKEY_CHPASSKEY,
			_("Change &passkey"), wxDefaultPosition, wxDefaultSize, 0);
	if (ShowToolTips())
		itemButton9->SetToolTip(
				_("Change key passpharse. You must know the old one."));
	itemBoxSizer2->Add(itemButton9, 0, wxALIGN_RIGHT | wxALL, 5);

	wxStaticBox *itemStaticBoxSizer10Static = new wxStaticBox(itemDialog1,
			wxID_ANY, _("Expiration"));
	wxStaticBoxSizer *itemStaticBoxSizer10 = new wxStaticBoxSizer(
			itemStaticBoxSizer10Static, wxHORIZONTAL);
	itemBoxSizer2->Add(itemStaticBoxSizer10, 1, wxGROW | wxALL, 5);

	m_expiryDate = new wxStaticText(itemDialog1, ID_EDITKEY_EXPIRYDATE,
	...
	wxButton *itemButton12 = new wxButton(itemDialog1, ID_EDITKEY_CHEXPIRATION,
			_("Change &expiration"), wxDefaultPosition, wxDefaultSize, 0);
	itemStaticBoxSizer10->Add(itemButton12, 0,
	...
	wxButton *itemButton15 = new wxButton(itemDialog1, wxID_OK, _("&OK"),
...
void wxGnuPGShellEditKey::OnEditkeyChpasskeyClick(wxCommandEvent &event) {
	...
	if (passDlg->ShowModal() == wxID_OK) {
		...
		if (wxGetApp().ChangePass(m_keyID->GetLabel(),
				passDlg->m_currPass->GetValue(),
				passDlg->m_passKey->GetValue())) {
			wxMessageBox(_("Passkey was changed successfully!"), _("Success"));
		} else {
			wxMessageBox(
					_(
							"Passkey was not changed due to errors. Look to the log windows for details."),
					_("Error"));
		}
	}
	passDlg->Destroy();
...
void wxGnuPGShellEditKey::OnEditkeyChexpirationClick(wxCommandEvent &event) {
	...
	if (expDlg->ShowModal() == wxID_OK) {
		wxString date =
				expDlg->m_expirationMode->GetSelection() > 0 ?
						expDlg->m_calendarCtrl->GetDate().FormatISODate() :
						wxT("0");
		m_expiryDate->SetLabel(
				expDlg->m_expirationMode->GetSelection() > 0 ?
						expDlg->m_calendarCtrl->GetDate().FormatISODate() :
						wxString(_("Never")));
		...
		if (pass != wxEmptyString) {
			if (wxGetApp().ChangeExpiration(m_keyID->GetLabel(), date, pass)) {
				wxMessageBox(_("Expiration date was changed successfully!"),
						_("Success"));
			} else {
				wxMessageBox(
						_(
								"Expiration date was not changed due to errors. Look to the log windows for details."),
						_("Error"));
