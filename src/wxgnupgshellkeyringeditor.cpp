...
wxGnuPGShellKeyringEditor::wxGnuPGShellKeyringEditor( wxWindow* parent, wxWindowID id, const wxString& caption, const wxPoint& pos, const wxSize& size, long style )
{
    Init();
...
bool wxGnuPGShellKeyringEditor::Create( wxWindow* parent, wxWindowID id, const wxString& caption, const wxPoint& pos, const wxSize& size, long style )
{
...
wxGnuPGShellKeyringEditor::~wxGnuPGShellKeyringEditor()
{
    ...
    wxAuiNotebook * noteBook = wxDynamicCast(m_keyProperty->GetParent(), wxAuiNotebook);
...
////@begin wxGnuPGShellKeyringEditor content construction
    ...
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu3, wxID_EXIT, _("E&xit\tAlt+F4"), _T(""), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("exit-16x16x32b.xpm")));
        menuItem->SetBitmap(bitmap);
        itemMenu3->Append(menuItem);
    }
    menuBar->Append(itemMenu3, _("&File"));
    wxMenu* itemMenu5 = new wxMenu;
    itemMenu5->Append(ID_SELECT_LANGUAGE, _("&Language..."), _("Select interface language"), wxITEM_NORMAL);
    itemMenu5->AppendSeparator();
    itemMenu5->Append(ID_MENU_CLEAR_LOG, _("&Clear log"), _T(""), wxITEM_NORMAL);
    itemMenu5->AppendSeparator();
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu5, ID_MENU_PREFERENCES, _("&Preferences..."), _("Program settings"), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("settings-16x16x32b.xpm")));
        menuItem->SetBitmap(bitmap);
    ...
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu11, ID_MENU_REFRESH, _("&Refresh\tF5"), _("Reread keyring"), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("refresh-16x16x32b.xpm")));
        menuItem->SetBitmap(bitmap);
        itemMenu11->Append(menuItem);
    }
    itemMenu11->Append(ID_MENU_UPDATE_TRUSTDB, _("Refresh &Trust DB"), _("Update trust information"), wxITEM_NORMAL);
    itemMenu11->AppendSeparator();
    itemMenu11->Append(ID_MENU_KEY_PUB, _("P&ublic Keyring"), _("Show keys in public keyring"), wxITEM_RADIO);
    itemMenu11->Check(ID_MENU_KEY_PUB, true);
    itemMenu11->Append(ID_MENU_KEY_SEC, _("&Secret Keyring"), _("Show keys in secret keyring"), wxITEM_RADIO);
    itemMenu11->AppendSeparator();
    itemMenu11->Append(ID_MENU_SELECT_ALL, _("Select &All\tCtrl+A"), _("Select all keys in current keyring"), wxITEM_NORMAL);
    itemMenu11->AppendSeparator();
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu11, ID_MENU_NEW_KEY, _("Generate &New...\tCtrl+N"), _("Generate new key"), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("new.xpm")));
        menuItem->SetBitmap(bitmap);
        itemMenu11->Append(menuItem);
    }
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu11, ID_MENU_DEL_KEY, _("&Delete...\tDel"), _("Delete selected keys from keyring"), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("delete-16x16x32b.xpm")));
        menuItem->SetBitmap(bitmap);
        itemMenu11->Append(menuItem);
    }
    itemMenu11->Enable(ID_MENU_DEL_KEY, false);
    itemMenu11->AppendSeparator();
    itemMenu11->Append(ID_MENU_EDIT_KEY, _("&Edit...\tCtrl+E"), _("Allow to change key properties"), wxITEM_NORMAL);
    itemMenu11->Enable(ID_MENU_EDIT_KEY, false);
    itemMenu11->Append(ID_MENU_OWNER_TRUST, _("Set &Owner Trust..."), _("Set or change owner trust of selected keys"), wxITEM_NORMAL);
    itemMenu11->AppendSeparator();
    itemMenu11->Append(ID_MENU_REVOKE_KEY, _("Revoke..."), _T(""), wxITEM_NORMAL);
    itemMenu11->AppendSeparator();
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu11, ID_MENU_IMPORT_KEY, _("&Import..."), _("Import keys from files"), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("retreive-16x16x32b.xpm")));
        menuItem->SetBitmap(bitmap);
        itemMenu11->Append(menuItem);
    }
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu11, ID_MENU_EXPORT_KEY, _("E&xport..."), _("Export selected keys to files"), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("send-16x16x32b.xpm")));
        menuItem->SetBitmap(bitmap);
        itemMenu11->Append(menuItem);
    }
    ...
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu30, ID_MENU_RETRIEVE_KEY, _("&Retrieve Keys..."), _("Get key from default keyserver by it's ID"), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("retreive-16x16x32b.xpm")));
        menuItem->SetBitmap(bitmap);
        itemMenu30->Append(menuItem);
    }
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu30, ID_MENU_SEND_KEYS, _("&Send Keys..."), _("Send selected keys to default keyserver"), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("send-16x16x32b.xpm")));
        menuItem->SetBitmap(bitmap);
        itemMenu30->Append(menuItem);
    }
    ...
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu33, ID_MENU_FILEMANAGER, _("&Filemanager"), _("Run filemanager for encrypt/decrypt files"), wxITEM_NORMAL);
        wxBitmap bitmap(itemFrame1->GetBitmapResource(wxT("file_manager-16x16x32b.xpm")));
        menuItem->SetBitmap(bitmap);
        itemMenu33->Append(menuItem);
    }
    itemMenu33->Append(ID_MENU_CLEAR_PASS_CACHE, _("&Clear Passphrase Cache"), _T(""), wxITEM_NORMAL);
    menuBar->Append(itemMenu33, _("&Tools"));
    wxMenu* itemMenu36 = new wxMenu;
    {
        wxMenuItem* menuItem = new wxMenuItem(itemMenu36, wxID_ABOUT, _("&About...\tF1"), _T(""), wxITEM_NORMAL);
    ...
    wxToolBar* itemToolBar38 = new wxToolBar( itemFrame1, ID_TOOLBAR_KEYRING_EDITOR, wxDefaultPosition, wxDefaultSize, wxTB_FLAT|wxTB_HORIZONTAL|wxTB_NODIVIDER );
    itemToolBar38->SetToolBitmapSize(wxSize(32, 32));
    wxBitmap itemtool39Bitmap(itemFrame1->GetBitmapResource(wxT("art/pen_yellow.ico")));
    wxBitmap itemtool39BitmapDisabled;
    itemToolBar38->AddTool(ID_KE_TOOL_EDIT, _("Edit"), itemtool39Bitmap, itemtool39BitmapDisabled, wxITEM_NORMAL, _("Edit"), wxEmptyString);
    wxBitmap itemtool40Bitmap(itemFrame1->GetBitmapResource(wxT("art/delete.ico")));
    wxBitmap itemtool40BitmapDisabled;
    itemToolBar38->AddTool(ID_KE_TOOL_DELETE, _("Delete"), itemtool40Bitmap, itemtool40BitmapDisabled, wxITEM_NORMAL, _("Delete"), wxEmptyString);
    wxBitmap itemtool41Bitmap(itemFrame1->GetBitmapResource(wxT("art/retreive.ico")));
    wxBitmap itemtool41BitmapDisabled;
    itemToolBar38->AddTool(ID_KE_TOOL_IMPORT, _("Import"), itemtool41Bitmap, itemtool41BitmapDisabled, wxITEM_NORMAL, _("Import"), wxEmptyString);
    wxBitmap itemtool42Bitmap(itemFrame1->GetBitmapResource(wxT("art/send.ico")));
    wxBitmap itemtool42BitmapDisabled;
    itemToolBar38->AddTool(ID_KE_TOOL_EXPORT, _("Export"), itemtool42Bitmap, itemtool42BitmapDisabled, wxITEM_NORMAL, _("Export"), wxEmptyString);
    itemToolBar38->AddSeparator();
    wxBitmap itemtool44Bitmap(itemFrame1->GetBitmapResource(wxT("art/refresh.ico")));
    wxBitmap itemtool44BitmapDisabled;
    itemToolBar38->AddTool(ID_KE_TOOL_REFRESH, _("Refresh"), itemtool44Bitmap, itemtool44BitmapDisabled, wxITEM_NORMAL, _("Refresh"), _("Refresh key list"));
    itemToolBar38->AddSeparator();
    wxBitmap itemtool46Bitmap(itemFrame1->GetBitmapResource(wxT("art/file_manager.ico")));
    wxBitmap itemtool46BitmapDisabled;
    itemToolBar38->AddTool(ID_KE_TOOL_FILES, _("Files"), itemtool46Bitmap, itemtool46BitmapDisabled, wxITEM_NORMAL, _("File Manager"), _("Open Filemanager"));
    itemToolBar38->Realize();
    itemFrame1->GetAuiManager().AddPane(itemToolBar38, wxAuiPaneInfo()
        .ToolbarPane().Name(_T("Pane1")).Top().Layer(10).LeftDockable(false).RightDockable(false).CaptionVisible(false).CloseButton(false).DestroyOnClose(false).Resizable(false).Floatable(false).Gripper(true));

    m_keyListCtrl = new wxGnuPGKeyListCtrl( itemFrame1, ID_KE_KEYLIST, wxDefaultPosition, wxSize(100, 100), wxLC_REPORT|wxLC_VIRTUAL );
    itemFrame1->GetAuiManager().AddPane(m_keyListCtrl, wxAuiPaneInfo()
        .Name(_T("Pane2")).Caption(_T("Public keyring")).Centre().MinSize(wxSize(-1, 200)).CloseButton(false).DestroyOnClose(false).Resizable(true).Floatable(false).PaneBorder(false));
    ...
    m_auiNotebook = new wxAuiNotebook( itemFrame1, ID_KE_MAIN_NOTEBOOK, wxDefaultPosition, wxSize(-1, 150), wxNO_BORDER );

    m_keyProperty = new wxPropertyGrid( m_auiNotebook, ID_KE_KEYPROPERTY, wxDefaultPosition, wxSize(100, 200), wxNO_BORDER );

    m_auiNotebook->AddPage(m_keyProperty, _("Properties"), false);

    m_logText = new wxTextCtrl( m_auiNotebook, ID_KE_LOGTEXT, _T(""), wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxTE_READONLY|wxNO_BORDER );
    m_logText->SetFont(wxFont(8, wxSWISS, wxNORMAL, wxNORMAL, false, wxT("Courier New")));

    m_auiNotebook->AddPage(m_logText, _("Log"), false);

    itemFrame1->GetAuiManager().AddPane(m_auiNotebook, wxAuiPaneInfo()
        .Name(_T("InfoBar")).Bottom().BestSize(wxSize(-1, 200)).CaptionVisible(false).CloseButton(false).DestroyOnClose(false).Resizable(true).Floatable(false).PaneBorder(false));

    GetAuiManager().Update();

    // Connect events and objects
    itemFrame1->Connect(ID_WXGNUPGSHELLKEYRINGEDITOR, wxEVT_DESTROY, wxWindowDestroyEventHandler(wxGnuPGShellKeyringEditor::OnDestroy), NULL, this);
    m_keyListCtrl->Connect(ID_KE_KEYLIST, wxEVT_CONTEXT_MENU, wxContextMenuEventHandler(wxGnuPGShellKeyringEditor::OnContextMenu), NULL, this);
////@end wxGnuPGShellKeyringEditor content construction
    wxLog * oldTarget = wxLog::SetActiveTarget(new wxLogTextCtrl(m_logText));
    wxDELETE(oldTarget);
    wxLogMessage(_("GUI Created"));

#ifndef __WXMSW__
    m_keyListCtrl->Connect(wxID_ANY, wxEVT_RIGHT_DOWN, wxMouseEventHandler(wxGnuPGShellKeyringEditor::OnKeyListRightDown), NULL, this);
#endif
    m_popupMenu = NULL;
...
void wxGnuPGShellKeyringEditor::OnMenuSelectAllClick( wxCommandEvent& event )
{
    ...
    for (int i = 0; i < n; i++) { m_keyListCtrl->SetItemState(i, select, wxLIST_STATE_SELECTED); }
}
...
void wxGnuPGShellKeyringEditor::OnMenuPreferencesClick( wxCommandEvent& event )
{
    ...
    if ( setDlg->ShowModal() == wxID_OK ) {
        wxGetApp().m_defaultKeyServer = setDlg->m_defServerCombo->GetString(setDlg->m_defServerCombo->GetSelection());
        wxGetApp().m_defKey = setDlg->m_keyListCtrl->GetItemText(setDlg->m_selectedKey);
        ...
        for (wxUint16 it = 0; it < setDlg->m_defServerCombo->GetCount(); it++) {
            ...
            if (item != wxEmptyString) { wxGetApp().m_serverList.Add(item); }
        }
    }
    setDlg->Destroy();
}
...
void wxGnuPGShellKeyringEditor::OnMenuDelKeyClick( wxCommandEvent& event )
{
    ...
    if (wxMessageBox(_("You really want to delete selected keys?"), _("Delete keys"), wxYES_NO) == wxYES) {
        delSecret = wxMessageBox(_("Delete secret keys also?"), _("Delete secret keys"), wxYES_NO) == wxYES;
        wxBusyInfo * info = new wxBusyInfo(_("Please wait, deleting keys..."), this);
        ...
        for (int i = static_cast<int>(list.GetCount()) - 1; i > -1 ; i--) {
            if (delSecret) {
                wxGetApp().DeleteKeyPair(wxGetApp().GetKeyId(list.Item(i)), wxGetApp().GetKeyFingerprint(list.Item(i)), list.Item(i));
            } else {
                wxGetApp().DeleteKey(wxGetApp().GetKeyId(list.Item(i)), list.Item(i));
            }
        }
        wxDELETE(info);
        RefreshKeyList();
        ...
        if (trustDlg->ShowModal() == wxID_OK) {
            ...
            if ( wxMessageBox(_("Do you really want to change owner trust?" ),
                _("Change owner trust"), wxYES_NO) == wxYES) {
                    if (wxGetApp().SetOwnerTrust(wxGetApp().GetKeyId(index), trust)) {
                        RefreshKeyList();
                    }
            }
        }
...
void wxGnuPGShellKeyringEditor::OnMenuExportKeyClick( wxCommandEvent& event )
{
    ...
    while (index != wxNOT_FOUND) {
        ...
        if (backup->ShowModal() == wxID_OK) {
            wxGetApp().ExportKey(wxGetApp().GetKeyId(index), backup->m_fileName->GetValue(), backup->m_isArmor->IsChecked());
        }
        index = m_keyListCtrl->GetNextSelected(index);
    }
    backup->Destroy();
...
void wxGnuPGShellKeyringEditor::OnMenuRetrieveKeyClick( wxCommandEvent& event )
{
    ...
    if (dlg->ShowModal() == wxID_OK) {
        ...
        wxString keyId;
        wxBusyInfo * info = new wxBusyInfo(_("Please wait, requesting key..."), this);

        while ( index != wxNOT_FOUND ) {
            keyId = dlg->m_resultKeyList->GetItemText(index);
            wxGetApp().RecvKeys(dlg->m_importServer->GetString(dlg->m_importServer->GetSelection()), keyId);
...
void wxGnuPGShellKeyringEditor::OnMenuSendKeysClick( wxCommandEvent& event )
{
    ...
    if (wxMessageBox(_("You really want to send your selected keys to default server?"), _("Send keys"), wxYES_NO) == wxYES) {
        long index = m_keyListCtrl->GetFirstSelected();
        while ( index != wxNOT_FOUND ) {
            keys.Append(wxGetApp().GetKeyId(index) + wxT(" "));
            index = m_keyListCtrl->GetNextSelected(index);
        }
        wxBusyInfo * info = new wxBusyInfo(_("Please wait, sending keys..."), this);
        bool res = wxGetApp().SendKeys(wxGetApp().m_defaultKeyServer, keys);
        wxDELETE(info);
        if (!res) {
            wxMessageBox(_("Error sending keys. See log for details"), _("Error sending keys"), wxOK);
        }
    }
}
...
void wxGnuPGShellKeyringEditor::UpdateProperties(const int index)
{
    ...
    if ( key ) {
        ...
        m_keyProperty->SetPropertyValue( wxT("ExpiresAt"),  key->GetExpireDateString());
        m_keyProperty->SetPropertyValue( wxT("CreatedAt"),  key->GetCreationDateString());
        m_keyProperty->SetPropertyValue( wxT("OwnerTrust"),  key->GetTrustString());
        m_keyProperty->SetPropertyValue( wxT("KeyType"),  key->Algorithm);
        m_keyProperty->SetPropertyValue( wxT("KeyValidity"),  key->GetValidityString());
    }
}
...
{
    wxBusyInfo * info = new wxBusyInfo(_("Please wait, refreshing keyring..."), this);
    wxSleep(1);
...
{
    event.Enable(wxGetApp().IsReady() && m_keyListCtrl->GetFirstSelected() > -1);
}
...
{
    event.Enable(wxGetApp().IsReady() && m_keyListCtrl->GetSelectedItemCount() == 1);
}
...
void wxGnuPGShellKeyringEditor::OnMenuUpdateTrustdbClick( wxCommandEvent& event )
{
    wxUnusedVar(event);
    RefreshKeyList();
}
...
void wxGnuPGShellKeyringEditor::OnMenuUpdateTrustdbUpdate( wxUpdateUIEvent& event )
{
    event.Enable(wxGetApp().IsReady());
}
...
void wxGnuPGShellKeyringEditor::OnMenuRetrieveKeyUpdate( wxUpdateUIEvent& event )
{
    event.Enable(wxGetApp().IsReady());
}
...
void wxGnuPGShellKeyringEditor::OnMenuFilemanagerUpdate( wxUpdateUIEvent& event )
{
    event.Enable(wxGetApp().IsReady());
}
...
{
    if (!m_popupMenu) { m_popupMenu = AppResources::CreateMenuMenu(); }
    int flags;
    if (m_keyListCtrl->HitTest(m_keyListCtrl->ScreenToClient(event.GetPosition()), flags, NULL) != wxNOT_FOUND ) {
...
void wxGnuPGShellKeyringEditor::OnKESTATBARUpdate( wxUpdateUIEvent& event )
{
    if ( wxGetApp().IsReady() ) {
        try {
            m_statusBar->SetStatusText(wxString::Format(_("Total keys: %d"), m_keyListCtrl->GetItemCount()), 0);
            m_statusBar->SetStatusText(wxString::Format(_("Selected keys: %d"), m_keyListCtrl->GetSelectedItemCount()), 1);
            return;
            m_statusBar->SetStatusText(wxString::Format(_("Default key: %s"), wxGetApp().m_defKey.GetData()), 2);
            m_statusBar->SetStatusText(wxString::Format(_("Status: %s"), wxGetApp().IsReady() ? _("Ready") : _("Failed")), 3);
        } catch (...) {}
    }
}
...
void wxGnuPGShellKeyringEditor::OnMENUREVOKEKEYClick( wxCommandEvent& event )
{
    ...
    while ( index != wxNOT_FOUND ) {
        ...
        if ( dlg->ShowModal() == wxID_OK ) {
            if (wxGetApp().GenRevoke(keyId, dlg->m_outputFile->GetValue(),
                wxString::Format(wxT("%d"),dlg->m_reasonRadio->GetSelection()),
                dlg->m_ownReason->GetValue(), dlg->m_passphrase->GetValue())) {
                    successCount++;
            }
    ...
    if (successCount > 0) {
        wxMessageBox(wxString::Format(_("Revocation certificates successfully generated for %d keys."), successCount), _("Finish"));
    } else {
        wxMessageBox(_("No revocation certificates has been generated."), _("Finish"));
    }
...
{
    event.Enable(m_keyListCtrl->GetSelectedItemCount() > 0 && !m_isPublicKeyring);
}
...
void wxGnuPGShellKeyringEditor::OnMENUCLEARPASSCACHEClick( wxCommandEvent& event )
{
