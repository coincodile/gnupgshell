...
bool wxGnuPGShellApp::OnInit()
{
    ...
    wxSetWorkingDirectory(path.GetPath(wxPATH_GET_SEPARATOR | wxPATH_GET_VOLUME));
#ifndef __WXMSW__
	...
	m_TranslationHelper = new wxTranslationHelper(*this,
		wxPathOnly(argv[0]) + wxFileName::GetPathSeparator() + wxT("languages"), true);
    RecreateGUI(false); // dont show

    m_gnuPGWrapper->GetKeyList().Clear();

    if ( IsReady() ) {
        wxLogMessage(_("Trying to list public keyring..."));
        ListPublicKeys();

        size_t count = m_gnuPGWrapper->GetKeyList().GetCount();
        if (!count) { // empty
            wxLogMessage(_("Sorry, but keyring is empty or missing. Starting key generation wizard..."));
            GenerateKey();
            //wxLogMessage(_("Trying to re-list public keyring..."));
            ListPublicKeys();
        } else {
            wxLogMessage(_("Found %ld public keys"), count);
        }
        // start cleanup timer. Interval = 5 sec
        m_timer = new wxTimer(this, wxID_CLEAR_TIMER);
        m_timer->Start(5000);
    } else {
        wxLogMessage(_("GnuPG was not found!"));
    }



	wxGnuPGShellKeyringEditor * topWindow = wxDynamicCast(GetTopWindow(), wxGnuPGShellKeyringEditor);
    if ( topWindow ) {
        topWindow->Show();
        topWindow->m_keyListCtrl->SetItemCount(static_cast<long>(m_gnuPGWrapper->GetKeyList().GetCount()));
	}
...
void wxGnuPGShellApp::OnClearTimer(wxTimerEvent & event)
{
    ...
    for (wxUint32 it = 0; it < GetKeyList().GetCount(); it++) {
        if ((now - GetKeyList()[it]->m_lastPassAccess).GetMinutes() > m_cacheTime ) {
...
void wxGnuPGShellApp::LoadServerList(wxConfig &config)
{
    ...
    if (config.GetFirstEntry(entry, index)) {
        do {
            ...
            if (serverAddr == wxEmptyString) { break; }
...
{
    bool res = false;
    wxGnuPGShellKeyGenWizard* wizard = new wxGnuPGShellKeyGenWizard(NULL, ID_WXGNUPGSHELLKEYGENWIZARD);
    if ( wizard ) {
        if (wizard->Run()) {
            // fill param
            TaskParams params;
            ...
            params[wxT("expire")] = (wizard->m_expire == wxEmptyString) ?  wxT("0") : wizard->m_expire;
            params[wxT("subkey")] = wizard->m_isSepareteSignKey ? wxT("Subkey-Type: ELG-E\nSubkey-Length: 1024\n") : wxT("");

            wxBusyInfo * info = new wxBusyInfo(_("Please wait, generating keys..."), GetTopWindow());
...
//
bool wxGnuPGShellApp::EncryptDocument(const wxString fileName, const wxString outFile, const wxString keyId, bool armored, bool overwrite)
{
    TaskParams params;
    params[wxT("file")] = fileName;
    wxLogMessage(wxT("in: %s"), fileName.GetData());
    params[wxT("out")] = outFile;
    wxLogMessage(wxT("out: %s"), outFile.GetData());
    params[wxT("KeyId")] = keyId;
    wxLogMessage(wxT("key: %s"), keyId.GetData());
    params[wxT("armor")] = armored ? wxT(" --armor ") : wxEmptyString;
    wxLogMessage(wxT("armor: %s"), params[wxT("armor")].GetData());
    params[wxT("overwrite")] = overwrite ? wxT("yes") : wxEmptyString;
    wxLogMessage(wxT("ow: %s"), params[wxT("overwrite")].GetData());
...
//
bool wxGnuPGShellApp::DecryptDocument(const wxString fileName, const wxString outFile, const wxString pass, bool overwrite)
{
    TaskParams params;
    params[wxT("file")] = fileName;
    params[wxT("out")] = outFile;
...
//
bool wxGnuPGShellApp::SignDocument(const wxString fileName, const wxString outFile, const wxString pass, bool armored)
{
    TaskParams params;
    params[wxT("file")] = fileName;
    params[wxT("out")] = outFile;
...
//
bool wxGnuPGShellApp::EncryptAndSignDocument(const wxString fileName, const wxString outFile, const wxString keyId, const wxString pass, bool armored, bool overwrite)
{
    TaskParams params;
    params[wxT("file")] = fileName;
    params[wxT("out")] = outFile;
...
void wxGnuPGShellApp::RecreateGUI(bool show)
{
	...
	wxGnuPGShellKeyringEditor* mainWindow = new wxGnuPGShellKeyringEditor( NULL, ID_WXGNUPGSHELLKEYRINGEDITOR );
...
bool wxGnuPGShellApp::DeleteKeyPair(const wxString keyId, const wxString fingerprint, const int index)
{
    TaskParams params;
    params[wxT("KeyId")] = keyId;
...
bool wxGnuPGShellApp::ChangePass(const wxString keyId, const wxString currPass, const wxString newPass)
{
    TaskParams params;
...
void wxGnuPGShellApp::ExportKey(const wxString keyId, const wxString fileName, bool armored)
{
    TaskParams params;
    params[wxT("KeyId")] = keyId;
...
bool wxGnuPGShellApp::ChangeExpiration(const wxString keyId, const wxString date, const wxString pass)
{
    TaskParams params;
    params[wxT("passphrase")] = pass;
...
bool wxGnuPGShellApp::SearchKeysOnServer(const wxString keyServer, const wxString searchKey, GpgKeyList & keyList)
{
    TaskParams params;
...
bool wxGnuPGShellApp::GenRevoke(const wxString keyId, const wxString outFile, const wxString reason, const wxString own, const wxString pass)
{
    TaskParams params;
    params[wxT("out")] = outFile;
    params[wxT("KeyId")] = keyId;
...
bool wxGnuPGShellApp::EncryptSymmetric(const wxString fileName, const wxString outFile, const wxString pass, bool armored)
{
    TaskParams params;
    params[wxT("out")] = outFile;
    params[wxT("file")] = fileName;
...
wxString wxGnuPGShellApp::GetPasswordForKey(const wxString keyId)
{
    ...
    if (index != wxNOT_FOUND) {
        if (GetKeyList()[index]->IsPassCacheEmpty()) {
            pass = wxGetPasswordFromUser(wxString::Format(_("Passphrase needed for key %s. Please, enter it."),keyId.GetData()), _("Passphrase needed"));
...
wxString wxGnuPGShellApp::GetPasswordForKey(const wxUint32 keyId)
{
    ...
    if (GetKeyList()[keyId]->IsPassCacheEmpty()) {
        pass = wxGetPasswordFromUser(wxString::Format(_("Passphrase needed for key %s. Please, enter it."), GetKeyList()[keyId]->m_key.Id.GetData()), _("Passphrase needed"));
    } else {
