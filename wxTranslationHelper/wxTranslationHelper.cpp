...
bool wxTranslationHelper::Load() {
	...
	if (m_UseNativeConfig) {
		...
		config = new wxFileConfig(m_App.GetAppName(), wxEmptyString,
				m_ConfigPath);
	}
	long language;
	config->SetPath(wxT("wxTranslation"));
	if (!config->Read(wxT("wxTranslationLanguage"), &language,
			wxLANGUAGE_UNKNOWN)) {
...
void wxTranslationHelper::GetInstalledLanguages(wxArrayString &names,
	...
	wxT("*.*"),
			...
			wxT("*"),
#endif  
			wxDIR_DEFAULT); cont; cont = dir.GetNext(&filename)) {
...
bool wxTranslationHelper::AskUserForLanguage(wxArrayString &names,
	...
	long index = wxGetSingleChoiceIndex(_("Select the language"), _("Language"),
			names);
	if (index != -1) {
		if (m_Locale) {
			wxDELETE(m_Locale);
