...
bool operator ==(const wxColourPropertyValue &a,
		const wxColourPropertyValue &b) {
...
WX_PG_IMPLEMENT_VALUE_TYPE(wxDateTime, wxDateProperty, wxT("datetime"),
		GetDateTime, wxDateTime())
...
#ifndef __WXPYTHON__
wxWindow* wxPGSpinCtrlEditor::CreateControls(wxPropertyGrid *propgrid,
...
wxPGWindowPair wxPGSpinCtrlEditor::CreateControls( wxPropertyGrid* propgrid, wxPGProperty* property,
	...
	propgrid->Connect(wxPG_SUBID2, wxEVT_SCROLL_LINEUP,
			(wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction) &wxPropertyGrid::OnCustomEditorEvent,
			NULL, propgrid);
...
#ifndef __WXPYTHON__
	wxTextCtrl *wnd1 = (wxTextCtrl*) wxPGTextCtrlEditor::CreateControls(
			propgrid, property, pos, tcSz, NULL);
...
bool wxPGSpinCtrlEditor::OnEvent(wxPropertyGrid *propgrid,
	...
	if (evtType == wxEVT_SCROLL_LINEUP || evtType == wxEVT_SCROLL_LINEDOWN) {
		...
		wxTextCtrl *tc = wxDynamicCast(propgrid->GetEditorControl(),
				wxTextCtrl);
		...
		wxSpinButton *spinButton =
				(wxSpinButton*) propgrid->GetEditorControlSecondary();
...
static const wxChar *gs_fp_es_family_labels[] = { wxT("Default"), wxT(
		"Decorative"), wxT("Roman"), wxT("Script"), wxT("Swiss"), wxT("Modern"),
		(const wxChar*) NULL };

static long gs_fp_es_family_values[] = { wxDEFAULT, wxDECORATIVE, wxROMAN,
		wxSCRIPT, wxSWISS, wxMODERN };

static const wxChar *gs_fp_es_style_labels[] = { wxT("Normal"), wxT("Slant"),
		wxT("Italic"), (const wxChar*) NULL };

static long gs_fp_es_style_values[] = { wxNORMAL, wxSLANT, wxITALIC };

static const wxChar *gs_fp_es_weight_labels[] = { wxT("Normal"), wxT("Light"),
		wxT("Bold"), (const wxChar*) NULL };

static long gs_fp_es_weight_values[] = { wxNORMAL, wxLIGHT, wxBOLD };

// Class body is in advprops.h

WX_PG_IMPLEMENT_PROPERTY_CLASS(wxFontProperty, wxBaseParentProperty, wxFont,
		const wxFont&, TextCtrlAndButton)

wxFontPropertyClass::wxFontPropertyClass(const wxString &label,
		const wxString &name, const wxFont &value) :
	...
	AddChild(
			wxIntProperty(_("Point Size"), emptyString,
					m_value_wxFont.GetPointSize()));

	AddChild(
			wxEnumProperty(_("Family"), emptyString, gs_fp_es_family_labels,
					gs_fp_es_family_values, m_value_wxFont.GetFamily()));
	...
	AddChild(
			wxEnumProperty(_("Style"), emptyString, gs_fp_es_style_labels,
					gs_fp_es_style_values, m_value_wxFont.GetStyle()));

	AddChild(
			wxEnumProperty(_("Weight"), emptyString, gs_fp_es_weight_labels,
					gs_fp_es_weight_values, m_value_wxFont.GetWeight()));

	AddChild(
			wxBoolProperty(_("Underlined"), emptyString,
...
void wxFontPropertyClass::ChildChanged(wxPGProperty *p) {
	...
	if (ind == 0) {
		...
		if (fam < wxDEFAULT || fam > wxTELETYPE)
			fam = wxDEFAULT;
		...
		m_value_wxFont.SetUnderlined(
				wxPGVariantToBool(p->DoGetValue()) ? true : false);
	}
}
...
static const wxChar *gs_cp_es_syscolour_labels[] = { wxT("AppWorkspace"), wxT(
		"ActiveBorder"), wxT("ActiveCaption"), wxT("ButtonFace"), wxT(
		"ButtonHighlight"), wxT("ButtonShadow"), wxT("ButtonText"), wxT(
		"CaptionText"), wxT("ControlDark"), wxT("ControlLight"), wxT("Desktop"),
		wxT("GrayText"), wxT("Highlight"), wxT("HighlightText"), wxT(
				"InactiveBorder"), wxT("InactiveCaption"), wxT(
				"InactiveCaptionText"), wxT("Menu"), wxT("Scrollbar"), wxT(
				"Tooltip"), wxT("TooltipText"), wxT("Window"), wxT(
				"WindowFrame"), wxT("WindowText"), wxT("Custom"),
		(const wxChar*) NULL };

static long gs_cp_es_syscolour_values[] = { wxSYS_COLOUR_APPWORKSPACE,
		wxSYS_COLOUR_ACTIVEBORDER, wxSYS_COLOUR_ACTIVECAPTION,
		wxSYS_COLOUR_BTNFACE, wxSYS_COLOUR_BTNHIGHLIGHT, wxSYS_COLOUR_BTNSHADOW,
		wxSYS_COLOUR_BTNTEXT, wxSYS_COLOUR_CAPTIONTEXT, wxSYS_COLOUR_3DDKSHADOW,
		wxSYS_COLOUR_3DLIGHT, wxSYS_COLOUR_BACKGROUND, wxSYS_COLOUR_GRAYTEXT,
		wxSYS_COLOUR_HIGHLIGHT, wxSYS_COLOUR_HIGHLIGHTTEXT,
		wxSYS_COLOUR_INACTIVEBORDER, wxSYS_COLOUR_INACTIVECAPTION,
		wxSYS_COLOUR_INACTIVECAPTIONTEXT, wxSYS_COLOUR_MENU,
		wxSYS_COLOUR_SCROLLBAR, wxSYS_COLOUR_INFOBK, wxSYS_COLOUR_INFOTEXT,
		wxSYS_COLOUR_WINDOW, wxSYS_COLOUR_WINDOWFRAME, wxSYS_COLOUR_WINDOWTEXT,
		wxPG_COLOUR_CUSTOM };
...
wxSystemColourPropertyClass::wxSystemColourPropertyClass(const wxString &label,
		const wxString &name, const wxColourPropertyValue &value) :
		wxEnumPropertyClass(label, name, gs_cp_es_syscolour_labels,
...
wxSystemColourPropertyClass::wxSystemColourPropertyClass(const wxString &label,
		const wxString &name, const wxChar **labels, const long *values,
		wxPGChoices *choicesCache, const wxColourPropertyValue &value) :
...
wxSystemColourPropertyClass::wxSystemColourPropertyClass(const wxString &label,
		const wxString &name, const wxChar **labels, const long *values,
		wxPGChoices *choicesCache, const wxColour &value) :
		wxEnumPropertyClass(label, name, labels, values, choicesCache) {
...
void wxSystemColourPropertyClass::DoSetValue(wxPGVariant value) {
	wxColourPropertyValue *pval = wxPGVariantToWxObjectPtr(value,
...
wxString wxSystemColourPropertyClass::GetValueAsString(int argFlags) const {
	...
	if (m_value.m_type == wxPG_COLOUR_CUSTOM
		...
		return wxString::Format(wxT("(%i,%i,%i)"), (int) m_value.m_colour.Red(),
				(int) m_value.m_colour.Green(), (int) m_value.m_colour.Blue());
	}
...
bool wxSystemColourPropertyClass::QueryColourFromUser(wxPropertyGrid *propgrid,
		wxWindow *primary) {
	bool res = false;
...
bool wxSystemColourPropertyClass::OnEvent(wxPropertyGrid *propgrid,
	...
	if (event.GetEventType() == wxEVT_COMMAND_COMBOBOX_SELECTED) {
		...
		if ((arrValues.GetCount() && type == wxPG_COLOUR_CUSTOM)
				|| (!arrValues.GetCount()
						&& (index == (int) (m_choices.GetCount() - 1)
...
bool wxSystemColourPropertyClass::SetValueFromString(const wxString &text,
		int flags) {
...
static const wxChar *gs_cp_es_normcolour_labels[] = { wxT("Black"), wxT(
		"Maroon"), wxT("Navy"), wxT("Purple"), wxT("Teal"), wxT("Gray"), wxT(
		"Green"), wxT("Olive"), wxT("Brown"), wxT("Blue"), wxT("Fuchsia"), wxT(
		"Red"), wxT("Orange"), wxT("Silver"), wxT("Lime"), wxT("Aqua"), wxT(
		"Yellow"), wxT("White"), wxT("Custom"), (const wxChar*) NULL };

static unsigned long gs_cp_es_normcolour_colours[] = { wxPG_COLOUR(0, 0, 0),
		wxPG_COLOUR(128, 0, 0), wxPG_COLOUR(0, 0, 128), wxPG_COLOUR(128, 0,
				128), wxPG_COLOUR(0, 128, 128), wxPG_COLOUR(128, 128, 128),
		wxPG_COLOUR(0, 128, 0), wxPG_COLOUR(128, 128, 0), wxPG_COLOUR(166, 124,
				81), wxPG_COLOUR(0, 0, 255), wxPG_COLOUR(255, 0, 255),
		wxPG_COLOUR(255, 0, 0), wxPG_COLOUR(247, 148, 28), wxPG_COLOUR(192, 192,
				192), wxPG_COLOUR(0, 255, 0), wxPG_COLOUR(0, 255, 255),
		wxPG_COLOUR(255, 255, 0), wxPG_COLOUR(255, 255, 255), wxPG_COLOUR(0, 0,
				0) };

WX_PG_IMPLEMENT_CUSTOM_COLOUR_PROPERTY_USES_WXCOLOUR2(wxColourProperty,
		wxColourPropertyClass, gs_cp_es_normcolour_labels, (const long*)NULL,
		gs_cp_es_normcolour_colours, TextCtrlAndButton)
...
static const wxChar *gs_cp_es_syscursors_labels[] = { wxT("Default"), wxT(
		"Arrow"), wxT("Right Arrow"), wxT("Blank"), wxT("Bullseye"), wxT(
		"Character"), wxT("Cross"), wxT("Hand"), wxT("I-Beam"), wxT(
		"Left Button"), wxT("Magnifier"), wxT("Middle Button"), wxT("No Entry"),
		wxT("Paint Brush"), wxT("Pencil"), wxT("Point Left"), wxT(
				"Point Right"), wxT("Question Arrow"), wxT("Right Button"), wxT(
				"Sizing NE-SW"), wxT("Sizing N-S"), wxT("Sizing NW-SE"), wxT(
				"Sizing W-E"), wxT("Sizing"), wxT("Spraycan"), wxT("Wait"), wxT(
				"Watch"), wxT("Wait Arrow"), (const wxChar*) NULL };

static long gs_cp_es_syscursors_values[] = { wxCURSOR_NONE, wxCURSOR_ARROW,
		wxCURSOR_RIGHT_ARROW, wxCURSOR_BLANK, wxCURSOR_BULLSEYE, wxCURSOR_CHAR,
...
wxCursorPropertyClass::wxCursorPropertyClass(const wxString &label,
		const wxString &name, int value) :
		wxEnumPropertyClass(label, name, gs_cp_es_syscursors_labels,
				gs_cp_es_syscursors_values, value) {
...
#else
void wxCursorPropertyClass::OnCustomPaint(wxDC&, const wxRect&,
		wxPGPaintData&) {
}
#endif
...
wxImageFilePropertyClass::wxImageFilePropertyClass(const wxString &label,
...
void wxImageFilePropertyClass::OnCustomPaint(wxDC &dc, const wxRect &rect,
		wxPGPaintData&) {
...
wxMultiChoicePropertyClass::wxMultiChoicePropertyClass(const wxString &label,
		const wxString &name, const wxPGChoices &choices,
		const wxArrayInt &value) :
...
wxMultiChoicePropertyClass::wxMultiChoicePropertyClass(const wxString &label,
		const wxString &name, const wxArrayString &strings,
		const wxArrayInt &value) :
		wxPGProperty(label, name) {
	wxPG_INIT_REQUIRED_TYPE(wxArrayInt)
...
wxMultiChoicePropertyClass::wxMultiChoicePropertyClass(const wxString &label,
		const wxString &name, const wxArrayInt&WXUNUSED(value)) :
		wxPGProperty(label, name) {
}
...
bool wxMultiChoicePropertyClass::OnEvent(wxPropertyGrid *propgrid,
	...
	if (event.GetEventType() == wxEVT_COMMAND_BUTTON_CLICKED) {
		...
		wxMultiChoiceDialog dlg(propgrid, _("Make a selection:"), m_label,
				m_choices.GetCount(), &m_choices.GetLabels()[0],
				wxCHOICEDLG_STYLE);
...
bool wxMultiChoicePropertyClass::SetValueFromString(const wxString &text, int) {
	...
	WX_PG_TOKENIZER2_BEGIN(text, wxT('"'))int ind = strings.Index( token );
	if ( ind != wxNOT_FOUND )
	{
		if ( values.GetCount() )
		ind = values.Item(ind);
...
WX_PG_IMPLEMENT_PROPERTY_CLASS(wxDateProperty, wxBaseProperty, wxDateTime,
		const wxDateTime&, dtCtrl)
...
bool wxDatePropertyClass::SetValueFromString(const wxString &text,
	...
	const wxChar *c = m_valueDateTime.ParseFormat(text.c_str(),
...
wxString wxDatePropertyClass::GetValueAsString(int argFlags) const {
	...
	if (m_format.length() && !(argFlags & wxPG_FULL_VALUE))
